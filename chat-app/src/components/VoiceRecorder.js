...
const VoiceRecorder = ({ onClose, handleSendFile }) => {
  ...
  const stopAndMergeChunks = () => {
    const superBuffer = new Blob(recordedChunks, { type: 'audio/wav' });
  
    if (audioURL) {
      fetch(audioURL)
        .then(response => response.blob())
        .then(previousBlob => compareBlobs(previousBlob, superBuffer));
    } else {
      finalizeRecording(superBuffer);
    }
  };
  ...
  const compareBlobs = async (blob1, blob2) => {
    ...
    if (buffer1.byteLength !== buffer2.byteLength) {
      finalizeRecording(blob2); // Sizes are different, no duplication.
      return;
    }
    ...
    for (let i = 0; i < view1.length; i++) {
      if (view1[i] !== view2[i]) {
        finalizeRecording(blob2); // Contents are different, no duplication.
        return;
      }
    }
  
    console.warn("Duplicate audio detected based on content.");
  };

  const finalizeRecording = (superBuffer) => {
  ...
  const {
    ...
    mediaBlobUrl,
    clearBlobUrl,
  } = useReactMediaRecorder({ audio: true, onDataAvailable: handleDataAvailable });
  ...
  const handleRecording = () => {
    if (isRecording) {
      if (isPaused) {
        ...
        stopAndMergeChunks(); // Merge chunks and update audio URL
